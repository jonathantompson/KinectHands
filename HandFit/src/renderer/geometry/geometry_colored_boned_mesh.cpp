...
namespace renderer {
  ...
  void GeometryColoredBonedMesh::syncVAO() {
    if (synced_) {
      wstring err(L"syncVBO() - ERROR: dynamic VBOs not yet supported");
    ...
    if (colors_.size() != vertices_.size() || 
      ...
      wstring err = wstring(L"syncVBO() - ERROR: vertex, colors bones and ") +
        wstring(L"normal vectors must be the same size!");
      throw wruntime_error(err);
    }
  ...
  void GeometryColoredBonedMesh::draw() {
    if (!synced_) {
      throw wruntime_error(wstring(L"GeometryColoredBonedMesh::draw() - ") + 
  ...
  data_str::Pair<uint8_t*,uint32_t> GeometryColoredBonedMesh::saveToArray() {

...
namespace renderer {
  ...
  void GeometryColoredMesh::syncVAO() {
    if (synced_) {
      wstring err = wstring(L"syncVBO() - ERROR: dynamic VBOs supported.  ") +
        wstring(L"call unsyncVBO() first.");
      throw wruntime_error(err);
    }

    if (colors_.size() != vertices_.size() || 
        colors_.size() != normals_.size()) {
      wstring err = wstring(L"syncVBO() - ERROR: vertex, color and normal ") +
        wstring(L"vectors must be the same size!");
      throw wruntime_error(err);
    }

    if (indices_.size() == 0) {
      if ((vertices_.size() % 3) != 0) {
        wstring err = wstring(L"syncVBO() - ERROR: num vertices must be a ") +
          wstring(L"multiple of 3 when not using the index array");
        throw wruntime_error(err);
      }
    }
  ...
  void GeometryColoredMesh::draw() {
    if (!synced_) {
      throw wruntime_error(wstring(L"GeometryColoredMesh::draw() - ") + 
        wstring(L"ERROR: trying to draw an unsynced vbo object!"));
    }
  ...
  float GeometryColoredMesh::calcSphereOutsideRadius(uint32_t n_stacks, 
    ...
    if(n_stacks < 3) {
      throw wruntime_error(wstring(L"GeometryColoredMesh::") +
        wstring(L"calcSphereOutsideRadius() - Error: n_stacks < 3"));
    }
    if(n_slices < 4) {
      throw wruntime_error(wstring(L"GeometryColoredMesh::") +
        wstring(L"calcSphereOutsideRadius()- Error: n_slices < 4"));
    }
  ...
  void GeometryColoredMesh::makeSphere(GeometryColoredMesh* ret, 
    ...
    if(n_stacks < 3) {
      throw wruntime_error(wstring(L"GeometryColoredMesh::makeSphere()") +
        wstring(L"- Error: n_stacks < 3"));
    }
    if(n_slices < 4) {
      throw wruntime_error(wstring(L"GeometryColoredMesh::makeSphere()") +
        wstring(L"- Error: n_slices < 4"));
    }
    ...
    if(cur_vertex != n_vert) {
      throw wruntime_error(wstring(L"GeometryColoredMesh::makeSphere() -") +
        wstring(L" Internal Error: Didn't create enough verticies"));
    }
#endif
    ...
    if(cur_index != n_ind) {
      throw wruntime_error(wstring(L"GeometryColoredMesh::makeSphere() -") +
        wstring(L" Internal Error: Didn't create enough indices"));
    }
#endif
  ...
  float GeometryColoredMesh::calcConeOutsideRadius(uint32_t n_slices, 
    ...
    if(n_slices < 3) {
      throw wruntime_error(wstring(L"GeometryColoredMesh::") +
        wstring(L"calcConeOutsideRadius()- Error: n_slices < 3"));
    }
  ...
  GeometryColoredMesh* GeometryColoredMesh::makeCone(uint32_t n_slices, 
    ...
    if(n_slices < 3) {
      throw wruntime_error(wstring(L"GeometryColoredMesh::makeCone()") +
        wstring(L"- Error: n_slices < 3"));
    }
    ...
    if(cur_vertex != n_vert) {
      throw wruntime_error(wstring(L"GeometryColoredMesh::makeCone() -") +
        wstring(L" Internal Error: Didn't create enough vertices"));
    }
#endif
  ...
  GeometryColoredMesh* GeometryColoredMesh::makeCylinder(uint32_t n_slices,
    ...
    if(n_slices < 3) {
      throw wruntime_error(wstring(L"GeometryColoredMesh::makeCylinder()") +
                           wstring(L"- Error: n_slices < 3"));
    }

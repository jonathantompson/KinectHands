...
#define CONSTANT_HASH(size, str) data_str::HashString(size, str)
#else
#define CONSTANT_HASH(size, str) \
  (data_str::StringHash(str).m_hash) % (size - 1)
#endif

#define DYNAMIC_HASH(size, str) data_str::HashString(size, str)

...
namespace hand_model {
  ...
  // Static variables
  HandModelFit* HandModelFit::cur_fit_ = NULL;
  HandModel* HandModelFit::cur_hand_[2] = {NULL, NULL};
  uint64_t HandModelFit::func_eval_count_ = 0;
  data_str::Vector<Coeff> HandModelFit::min_pts_;
  bool HandModelFit::include_min_pts_constraints_ = true;
  data_str::VectorManaged<Eigen::MatrixXf> HandModelFit::tiled_coeffs_;
  data_str::Vector<float> HandModelFit::tiled_residues_;
  
  const float HandModelFit::finger_crossover_penalty_threshold = -0.12566f;  // ~ 5 deg
 
  HandModelFit::HandModelFit(HandModelRenderer* hand_renderer, uint32_t num_hands) {
    if (num_hands > 2) {
      throw std::runtime_error("HandModelFit::HandModelFit() - num_hands > 2");
    }
  ...
  void HandModelFit::fitModel(int16_t* depth, uint8_t* label, HandModel* hands[2]) {
    min_pts_.resize(0);
    cur_hand_[0] = hands[0];
    coeff_.block<1, HAND_NUM_COEFF>(0, 0) = cur_hand_[0]->coeff();
    if (nhands_ > 1) {
      cur_hand_[1] = hands[1];
      coeff_.block<1, HAND_NUM_COEFF>(0, HAND_NUM_COEFF) = cur_hand_[1]->coeff();
    }
    cur_fit_ = this;
    ...
    if (nhands_ == 1) {
      cur_hand_[0]->coeff() = coeff_;
    } else {
      cur_hand_[0]->coeff() = coeff_.block<1, HAND_NUM_COEFF>(0, 0);
      cur_hand_[1]->coeff() = coeff_.block<1, HAND_NUM_COEFF>(0, HAND_NUM_COEFF);
    }
  }

  float HandModelFit::queryObjectiveFunction(int16_t* depth, uint8_t* label, 
    HandModel* hands[2]) {
    cur_hand_[0] = hands[0];
    coeff_.block<1, HAND_NUM_COEFF>(0, 0) = cur_hand_[0]->coeff();
    if (nhands_ > 1) {
  ...
  char* float2CStr(float val) {
    string str = string_util::Num2Str<float>(val);
  ...
  void HandModelFit::evalFunc(Eigen::MatrixXf& f_val, const Eigen::MatrixXf& coeff, 
  ...
  void HandModelFit::preturbCoeffs(Eigen::MatrixXf& coeff) {
    for (uint32_t i = 0; i < HAND_NUM_COEFF; i++) {
      float norm_rand_num_ = dist(eng);
      coeff(i) = coeff(i) + norm_rand_num_ * preturb_coeff_[i];
    }
    HandModel::renormalizeCoeffs(coeff);
  }

  float HandModelFit::calculateResidual(const Eigen::MatrixXf& y, 
  ...
  void HandModelFit::approxJacobianTiled(MatrixXf& jacob, const MatrixXf& coeff) {
    for (uint32_t frame = 0; frame < cur_fit_->coeff_dim_ / HAND_NUM_COEFF; frame++) {
      ...
      for (uint32_t i = c_start; i < c_end; i++) {
        ...
        HandModel::renormalizeCoeffs(cur_fit_->coeff_tmp_);
  ...
  void HandModelFit::approxJacobian(MatrixXf& jacob, const MatrixXf& coeff) {
    cur_fit_->coeff_tmp_ = coeff;
  ...
  float HandModelFit::calcPenalty() {
    return calcPenalty(coeff_);
  }

  float HandModelFit::calcPenalty(const Eigen::MatrixXf& coeff) {
    float penalty = 1.0f;
    for (uint32_t i = 0; i < cur_fit_->coeff_dim_; i++) {
  ...
  float HandModelFit::evaluateFuncAndCalculateResidual() {
    MatrixXf x;
    MatrixXf y;
    MatrixXf f;
    evalFunc(f, coeff_, x);
  ...
  float HandModelFit::objectiveFunc(const MatrixXf& coeff) {
    MatrixXf x;
    MatrixXf y;
    MatrixXf f;
    evalFunc(f, coeff, x);
  ...
  void HandModelFit::objectiveFuncTiled(data_str::Vector<float>& residues, 
    data_str::VectorManaged<Eigen::MatrixXf>& coeffs) {
  ...
  // jacobian.
  const float HandModelFit::delta_coeff_[HAND_NUM_COEFF] = {
    1.0f,    // HAND_POS_X
    1.0f,    // HAND_POS_Y
    1.0f,    // HAND_POS_Z
    0.01f,   // HAND_ORIENT_X
  ...
  const float HandModelFit::preturb_coeff_[HAND_NUM_COEFF] = {
    1.0f,    // HAND_POS_X
    1.0f,    // HAND_POS_Y
    1.0f,    // HAND_POS_Z
    0.01f,  // HAND_ORIENT_X
  ...
  // a pure angle (0 --> 2pi)
  const bool HandModelFit::angle_coeffs[HAND_NUM_COEFF*2] = {
    // Hand 1
    false,  // HAND_POS_X
    false,  // HAND_POS_Y
    false,  // HAND_POS_Z
  ...
  const float HandModelFit::coeff_min_limit[HAND_NUM_COEFF] = {
  ...
  // function heavily penalizes configurations with this value
  const float HandModelFit::coeff_max_limit[HAND_NUM_COEFF] = {
    std::numeric_limits<float>::infinity(),    // HAND_POS_X
    std::numeric_limits<float>::infinity(),    // HAND_POS_Y
    std::numeric_limits<float>::infinity(),    // HAND_POS_Z
    1,  // HAND_ORIENT_X
  ...
  const uint32_t HandModelFit::manual_fit_order_[HAND_NUM_COEFF] = {
    HAND_POS_X,
    HAND_POS_Y,
    HAND_POS_Z,
    HAND_ORIENT_X,
  ...
  const float HandModelFit::coeff_penalty_scale_[HAND_NUM_COEFF] = {
    0,    // HAND_POS_X
    0,    // HAND_POS_Y
    0,    // HAND_POS_Z
    0,  // HAND_ORIENT_X
  ...
  const float HandModelFit::save_2_disk_coeff_radius[HAND_NUM_COEFF] = {
    30.0f,   // HAND_POS_X
    30.0f,   // HAND_POS_Y
    30.0f,   // HAND_POS_Z
    0.5f,   // HAND_ORIENT_X
  ...
  const int HandModelFit::save_2_disk_sample_radius[HAND_NUM_COEFF] = {
    0,       // HAND_POS_X
    0,       // HAND_POS_Y
    0,       // HAND_POS_Z
    0,       // HAND_ORIENT_X
  ...
  void Coeff::copyFromEigen(const Eigen::MatrixXf &c) {
    for (uint32_t i = 0; i < c.cols() || i < c.rows(); i++) {
      this->c[i] = c(i);
    }
  }

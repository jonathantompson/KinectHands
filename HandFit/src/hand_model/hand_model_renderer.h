...
namespace hand_model {
  ...
  public:
    ...
    HandModelRenderer(renderer::Renderer* g_renderer, bool left, bool right);
    ~HandModelRenderer();

    // Call before rendering hand depth maps:
    void uploadKinectDepth(int16_t* kinect_depth);  

    void updateMatrices(const Eigen::MatrixXf& coeff, HandType hand_type);
    void visualizeDepthMap(windowing::Window* wnd, bool color = false);
    void drawDepthMap(const Eigen::MatrixXf& coeff, HandModel** hands, 
      uint32_t num_hands, bool color = false);
    void drawDepthMapTiled(data_str::Vector<float>& residues,
      data_str::VectorManaged<Eigen::MatrixXf>& coeff, HandModel** hands, 
    ...
    inline HandModelGeometry* r_hand_geom() { return r_hand_geom_; }
  ...
  private:
    ...
    HandModelGeometry* l_hand_geom_;
    HandModelGeometry* r_hand_geom_;
    ...
    data_str::Vector<renderer::BoundingSphere*> bsph_;  // Not owned here
    ...
    data_str::Vector<uint32_t> active_list_;
    data_str::Vector<data_str::Pair<uint32_t, uint32_t>> collisions_;
    ...
    void drawDepthMapInternal(const Eigen::MatrixXf& coeff, HandModel** hands, 
      uint32_t num_hands, bool color, bool tiled);

    // Non-copyable, non-assignable.
    HandModelRenderer(HandModelRenderer&);
    HandModelRenderer& operator=(const HandModelRenderer&);
  };

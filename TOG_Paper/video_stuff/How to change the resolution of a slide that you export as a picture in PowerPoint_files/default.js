...
function MS_StatsDotNet() {
    ...
    this.HookLoadEvents = function () {
        ...
        $(document.body).mousemove(StatsDotNet.ResetDwellTime);
    ...
    this.LogPageOnLoad = function () {
        ...
        ltsUrl += 'SSID=' + OutputEncoder_EncodeUrl(this.ssId) + '&' +
                ...
                'PassportStatus=' + OutputEncoder_EncodeUrl(this.PassportStatus) + '&' +
    ...
    this.LogPageOnUnLoad = function () {
        ...
        $('#StatsDotNetImg').get(0).src = ltsUrl;
        $("meta[name='ms.EventCollectionId']").attr("content", StatsDotNet.eventCollectionId);
        if (typeof (MscomCustomEvent) != "undefined") {
            MscomCustomEvent(
                        "ms.ExitLinkID", OutputEncoder_EncodeUrl(StatsDotNet.exitLinkId),
                        "ms.ExitContainerId", OutputEncoder_EncodeUrl(StatsDotNet.exitContainerId),
                        "ms.IsUnLoad", OutputEncoder_EncodeUrl(true),
                        "ms.DwellTime", OutputEncoder_EncodeUrl(StatsDotNet.dwellTime));
        }
        var today = new Date();
    ...
    this.SetExitLinks = function (e) {
        ...
        if (El.target.toUpperCase() === '_BLANK') {
            this.targetUrl = '';
        }
...
function recordLinkClick(flexValueId, id) {
    if (id) {
        var value = StatsDotNet['flexValue' + flexValueId];
        value = $.trim(value);
        if (!value) {
            value = id;
        } else {
        ...
        StatsDotNet['flexValue' + flexValueId] = value;
    }
}
...
function generateWedcsData() {
    ...
    var sessionId = fetchcookieval("GsfxSessionCookie");
    var metas = [buildMeta("eventseqno", inc),
     buildMeta("SsVersion", StatsDotNet.SsVersion),
     buildMeta("EventCollectionId", StatsDotNet.eventCollectionId),
     buildMeta("OptionCollectionId", StatsDotNet.OptionCollectionId),
     buildMeta("ContentProperties", StatsDotNet.ContentProperties),
     buildMeta("ContentLcid", StatsDotNet.contentLn),
     buildMeta("BrandId", StatsDotNet.siteBrandId),
     buildMeta("Intended404Url", StatsDotNet.In404Url),
     buildMeta("ReferrerContentType", StatsDotNet.rctype),
     buildMeta("ReferrerContentLcid", StatsDotNet.rclcid),
     buildMeta("ReferrerContentCulture", StatsDotNet.rcculture),
     buildMeta("ReferrerContentId", StatsDotNet.rcid),
     buildMeta("ReferrerExitLinkID", StatsDotNet.rexlid),
     buildMeta("ReferrerExitContainerID", StatsDotNet.rexcid),
     buildMeta("SessionId", sessionId)].join("");
    $(metas).appendTo($('head'));
}

var buildMeta = function (name, content) {
    return (content === undefined || content === null) ? '' : '<meta name="ms.' + name + '" content="' + content + '" />';
};

$(function () {
    $('.ShareButton').css("display", "block");
    var slideUpMediaContent = function () {
        $(".MediaContent ul").slideUp("fast");
    }
    $('.ShareButton').click(function () {
        $(".MediaContent ul").slideToggle("fast");
        return (false);
    });
    $(document).bind("click", slideUpMediaContent);
    $(".MediaContent").unbind("click", slideUpMediaContent);
});

var GSS = GSS || {}; GSS.Utility = GSS.Utility || {};
(function ($) {
    function URI(str) {
        if (!str) {
            str = "";
        ...
        if (this.query) {
            ...
            for (var i = 0; i < pairs.length; i++) {
                var pair = pairs[i].split('='), key, value;
                if (pair.length != 2) { continue; }
                if (pair[1] == '') { continue; }
                try {
                    ...
                    //BUGBUG: What this "catch" for?
                    key = unescape(pair[0]).toLowerCase();
        ...
        this.modifyQueryItems = function (data) {
            $.extend(this.queryString, data);
        };
    }
    ...
    URI.prototype = {
        ...
        toString: function () {
            ...
            this.query = $.param(this.queryString);
            if (this.query) {
                str += "?" + $.param(this.queryString);
            }
            if (this.fragment) {
                str += "#" + this.fragment;
            }
    ...
    var backupPathArray;
    var cnt = -1;
    var checkImageExistence = function (elementId, path) {
        $.ajax({
            url: path,
            type: "HEAD",
            processData: false,
            success: function () {
                $("#" + elementId).attr("src", path);
                $("#" + elementId).show();
            },
            statusCode: {
                404: function () {
                    if (cnt === (backupPathArray.length - 1)) {
                        $("#" + elementId).remove();
                        return;
                    }
                    cnt = cnt + 1;
                    checkImageExistence(elementId, backupPathArray[cnt]);
                }
            }
        });
    };

    GSS.Utility.SetImageSrc = function (elementId, backupPathList, defaultPath) {
        backupPathArray = backupPathList.split("|");
        checkImageExistence(elementId, defaultPath);
    };

    GSS.Utility.DisplaySsbAd = function () {
    ...
    GSS.Utility.DisplaySsbBanner = function () {
        var muid = fetchcookieval("MUID");
        if (!muid) {
            return;
        }
    ...
    GSS.Utility.TryRedirectToSsb = function (url) {
        var ssbVisited = fetchcookieval("ssbvisited");
        var ssbReminded = fetchcookieval("ssbreminder");
        var muid = fetchcookieval("MUID");
        if (ssbVisited === null && ssbReminded === null && muid) {
            isSmallMediumBusiness(muid, function () { self.location = url; });
        }
    };

    var isSmallMediumBusiness = function (muid, callback) {
        $.get("/content/OptService.ashx", null,
                function (data) {
                    logSsbInfo(muid, data);
    ...
    var logSsbInfo = function (muid, types) {
        var requestDate = new Date();
        var requestDateS = requestDate.toDateString();
        var reportingStr = "SSID=25&RequestDate=" + requestDateS +
            "&Url=" + encodeURIComponent(StatsDotNet.url) +
                "&Flexid=1&Flexvalue2=SSBOPT" +
                    "&Flexvalue3=" + muid +
                        "&Flexvalue4=" + types +
                            "&SiteCulture=" + StatsDotNet.SiteCulture +
                                "&ContentCulture=" + StatsDotNet.ContentCulture +
                                    "&ContentID=" + StatsDotNet.contentId +
                                        "&ContentType=" + StatsDotNet.contentType +
                                            "&ActiveDwellTime=" + StatsDotNet.activeDwellTime +
                                                "&EventSeqNo=" + StatsDotNet.eventSeqNo;
        $.ajax({ url: "/lts/default.aspx", data: reportingStr });
    };

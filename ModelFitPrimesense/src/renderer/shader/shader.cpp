...
namespace renderer {
  ...
  Shader::Shader(const string& filename, ShaderType type) {
    ...
    if(is_compiled == 0) {
      ...
      throw wruntime_error(wstring(L"Shader::Shader() - ERROR compiling") +
        wstring(L" shader from file: ") + filename_ + wstring(L": ") + 
        err_log);
    } else {
      compiled_ = true;
    }
  ...
  string Shader::extractIncludeFilename(GLchar* source, int pos) {
    ...
    throw wruntime_error(wstring(L"Shader::extractIncludeFilename() - ERROR") +
      wstring(L" Check couldn't extract include filename!")); 
  }
  ...
  GLchar* Shader::insertIncludeSource(GLchar* source, GLchar* inc, 
    ...
    if (inc_pos > source_length) {
      throw wruntime_error(wstring(L"Shader::insertIncludeSource() - ERROR") +
      wstring(L" inc_pos > source_length!")); 
    }

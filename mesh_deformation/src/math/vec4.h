...
namespace math {
  ...
  class DATA_ALIGN(ALIGNMENT) Vec4 {
   public:
    ...
    inline void trans_mult(Vec4* a, Mat4x4<T>* b) { trans_mult(this, a, b); }
    inline void trans_mult(Mat4x4<T>* a, Vec4* b) { trans_mult(this, a, b); }
    ...
    static void trans_mult(Vec4* ret, Vec4* a, Mat4x4<T>* b);  // this = a^t*b
    static void trans_mult(Vec4* ret, Mat4x4<T>* a, Vec4* b);  // this = a^t*b
  ...
  void Vec4<T>::mult(Vec4* ret, Mat4x4<T>* a, Vec4* b) {
    ...
    ret->m[1] = a->m[4]*b->m[0]  + a->m[5]*b->m[1]  + 
      a->m[6]*b->m[2]  + a->m[7]*b->m[3];
    ret->m[2] = a->m[8]*b->m[0]  + a->m[9]*b->m[1]  + 
      a->m[10]*b->m[2] + a->m[11]*b->m[3];
    ret->m[3] = a->m[12]*b->m[0] + a->m[13]*b->m[1] + 
      a->m[14]*b->m[2] + a->m[15]*b->m[3];
  };

  template <class T>
  void Vec4<T>::trans_mult(Vec4* ret, Vec4* a, Mat4x4<T>* b) {
    ret->m[0] = a->m[0]*b->m[0] + a->m[1]*b->m[4] + 
      a->m[2]*b->m[8]  + a->m[3]*b->m[12];
    ret->m[1] = a->m[0]*b->m[1] + a->m[1]*b->m[5] + 
      a->m[2]*b->m[9]  + a->m[3]*b->m[13];
    ret->m[2] = a->m[0]*b->m[2] + a->m[1]*b->m[6] + 
      a->m[2]*b->m[10] + a->m[3]*b->m[14];
    ret->m[3] = a->m[0]*b->m[3] + a->m[1]*b->m[7] + 
      a->m[2]*b->m[11] + a->m[3]*b->m[15];
  };

  template <class T>
  void Vec4<T>::trans_mult(Vec4* ret, Mat4x4<T>* a, Vec4* b) {
    ret->m[0] = a->m[0]*b->m[0] + a->m[4]*b->m[1] + 
      a->m[8]*b->m[2]  + a->m[12]*b->m[3];
    ret->m[1] = a->m[1]*b->m[0] + a->m[5]*b->m[1] + 
      a->m[9]*b->m[2]  + a->m[13]*b->m[3];
    ret->m[2] = a->m[2]*b->m[0] + a->m[6]*b->m[1] +
      a->m[10]*b->m[2] + a->m[14]*b->m[3];
    ret->m[3] = a->m[3]*b->m[0] + a->m[7]*b->m[1] + 

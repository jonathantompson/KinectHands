...
namespace math {
  ...
  MERSINE_TWISTER_ENG LPRPSOFittingModelFit::eng;
  UNIFORM_REAL_DISTRIBUTION LPRPSOFittingModelFit::dist_real(0, 1);

  LPRPSOFittingModelFit::LPRPSOFittingModelFit(uint32_t num_coeffs, 
    int swarm_size, ModelFit* model_fit, CoeffUpdateFuncPtr coeff_update_func) {
    if (swarm_size % 64 != 0) {
    ...
    for (uint32_t i = 0; i < NTILES; i++) {
      tiled_coeffs[i] = new float[num_coeffs_];
    }
    tiled_residues.capacity(NTILES);
    tiled_residues.resize(NTILES);
  }

  LPRPSOFittingModelFit::~LPRPSOFittingModelFit() {
    if (swarm_) {
      for (uint32_t i = 0; i < swarm_size_; i++) {
  ...
  void LPRPSOFittingModelFit::fitModel(const float* end_c,
    const float* start_c, const float* radius_c, const bool* angle_coeffs) {
    eng.seed();
    ...
    for (uint32_t i = 0; i < (swarm_size_ / NTILES); i++) {
      ...
      model_fit_->objectiveFuncTiled(tiled_residues, tiled_coeffs);
      for (uint32_t j = 0; j < NTILES; j++) {
  ...
  //float LPRPSOFittingModelFit::interpolateCoeff(const float a, 
  ...
  float LPRPSOFittingModelFit::calcDisplacement(const float a, const float b, 
    const bool angle) {
    float disp;
    if (angle) {
  ...
  void LPRPSOFittingModelFit::InsertionSortSwarmPts() {
    for (uint32_t i = 1; i < swarm_size_; i++) {

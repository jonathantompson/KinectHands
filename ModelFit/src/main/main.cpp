...
void KeyboardCB(int key, int scancode, int action, int mods) {
  ...
  switch (key) {
    ...
    case static_cast<int>('P'):
      if (action == RELEASED) {
        if (shift_down) {
          perform_zoom = !perform_zoom;
          cout << "perform_zoom = " << perform_zoom << std::endl;
        } else {
    ...
    case 'N':
      if (action == RELEASED) {
        if (video_stream) {
          ...
          std::wstring file = L"model_fit_video.avi";
  ...
  try {
    ...
    while (!shutting_down) {
      ...
      if (perform_zoom) {
        ...
        float val;
        if (zoom_time * zoom_speed < 6) {
          val = 1.0f / (1.0f + expf(-zoom_time * zoom_speed));
        } else {
        ...
        Float3* eye_pos = render->camera()->eye_pos();
        eye_pos->set(cur_pos);

        FloatQuat rot_end(0.084658f, 0.33448f, -0.030185f, 0.93811f);
        FloatQuat rot_start(0,0,0,1);
        FloatQuat delta_rot;
        FloatQuat::sub(delta_rot, rot_end, rot_start);
        FloatQuat::scale(delta_rot, val);
        FloatQuat cur_rot;
        FloatQuat::add(cur_rot, delta_rot, rot_start);

        render->camera()->eye_rot()->set(cur_rot);

        if (zoom_time * zoom_speed > 18) {
          perform_zoom = false;
          std::cout << "perform_zoom = " << perform_zoom << std::endl;
          zoom_time -= 24 / zoom_speed;
        }
      }

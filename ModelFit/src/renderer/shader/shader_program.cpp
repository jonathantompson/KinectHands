...
namespace renderer {
  ...
  ShaderProgram::ShaderProgram(Shader* vertex_shader, 
    ...
    if (!vertex_shader->compiled() || !fragment_shader->compiled()) {
      throw wruntime_error(wstring(L"ShaderProgram::ShaderProgram() - ERROR") +
        wstring(L": one of the shaders is not compiled!"));
    }
  ...
  void ShaderProgram::link() {
    ...
    if(is_linked == 0) {
      ...
      throw wruntime_error(wstring(L"ShaderProgram::ShaderProgram() - ERROR ") + 
        wstring(L"compiling shader program from file: ") + 
        vertex_shader_->filename() + wstring(L" and ") + 
        fragment_shader_->filename() + wstring(L": ") + err_log);
      return;
    } else {
      linked_ = true;
    }
  ...
  void ShaderProgram::bindFragShaderOutputLocation(
    ...
    if (linked_) {
      throw wruntime_error(wstring(L"ShaderProgram::bindFragShaderOutput") +
        wstring(L"Location() - Error, trying to bind a frag shader location") +
        wstring(L" on a shader program that is already linked"));
    }
#endif
  ...
  void ShaderProgram::bindVertShaderInputLocation(
    ...
    if (input.id >= n_vertex_attrib) {
      throw wruntime_error(wstring(L"ShaderProgram::bindFragShaderOutput") +
        wstring(L"Location() - Error, vertex attribute location >= ") +
        wstring(L" GL_MAX_VERTEX_ATTRIBS"));
    }
...
#if defined(DEBUG) || defined(_DEBUG)
    if (!linked_) {
      throw wruntime_error(wstring(L"ShaderProgram::useProgram() - ") +
        wstring(L"Error, trying to use a shader program that is not linked!"));
    }
#endif
  ...
  GLint ShaderProgram::getUniformLocation(const char* name) {
#if defined(DEBUG) || defined(_DEBUG)
    if (!linked_) {
      throw wruntime_error(wstring(L"ShaderProgram::getUniformLocation() - ") +
        wstring(L"Error, shader program is not linked!"));
    }
#endif
    ...
    if (static_cast<uint32_t>(Location) == 0xFFFFFFFF) {
      ...
      ss << L"ShaderProgram::getUniformLocation() - ERROR: Unable to get the";
      ss << L" location of uniform '" << name << "'";

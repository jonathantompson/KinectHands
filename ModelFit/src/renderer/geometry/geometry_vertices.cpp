...
namespace renderer {
  ...
  void GeometryVertices::draw() {
    if (!synced_) {
      throw wruntime_error(wstring(L"GeometryVertices::draw() - ") + 
  ...
  Geometry* GeometryVertices::copy() {
    throw std::runtime_error(std::string("GeometryVertices::copy() - ") +
      std::string("ERROR: copy not yet implemented for this Geometry type"));
  }

...
namespace renderer {
  GLStateElem::GLStateElem(const GLenum state_name, const void* data, 
    ...
    if (size > MAX_DATA_SIZE) {
      throw std::wruntime_error("GLStateElem::GLStateElem() - max data size"
  ...
  void GLState::initGLState() {
    ...
    if (sizeof(dummy_enum) != GLTYPE_ENUM_SIZE ||
      ...
      startup_mutex_.unlock();
      throw wruntime_error(L"GLState::GLState() - ERROR: type size mismatch!");
    }
#endif
    ...
    if (max_texture_units_ > MAX_NUM_TEXTURE_UNITS) {
      startup_mutex_.unlock();
      throw wruntime_error(L"GLState::GLState() - ERROR: max_texture_units_ >"
        L"MAX_NUM_TEXTURE_UNITS, consider increasing size.");
    }
  ...
  void GLState::glsBindFramebuffer(const GLenum target, 
    ...
    case GL_DRAW_FRAMEBUFFER:
      ...
      break;
    default:
      throw std::wruntime_error("GLState::glsBindFramebuffer() - target"
        " invalid");
    }
    ERROR_CHECK;
  }
  ...
  void GLState::glsStencilOpSeparate(const GLenum face, const GLenum sfail, 
    ...
    case GL_BACK:
      ...
      break;
    default:
      throw std::wruntime_error("GLState::glsStencilOpSeparate() - face"
        " invalid");
    }
  }
  ...
  void GLState::glsStencilFuncSeparate(const GLenum face, 
    ...
    case GL_BACK:
      ...
      break;
    default:
      throw std::wruntime_error("GLState::glsStencilFuncSeparate() - face"
        " invalid");
    }
  }
  ...
  GLStateTexture GLState::GLTarget2GLStateTexture(const GLenum target) {
    ...
    default:
      throw std::wruntime_error("GLState::GLTarget2GLStateTexture() - "
        "target invalid");
    }
  }
  ...
  GLenum GLState::GLStateTexture2GLTargetBinding(const GLStateTexture target) {
    ...
    default:
      throw std::wruntime_error("GLState::GLStateTexture2GLTargetBinding() - "
        "target invalid");
    }
  }
  ...
  void GLState::getGLenum(const GLenum state_name, void* ret_data, 
    ...
    if (cur_elem->size != size || cur_elem->type != type) {
      state_data_mutex_.unlock();
      throw std::wruntime_error("GLState::getGLenum() - ERROR: Retrieved state"

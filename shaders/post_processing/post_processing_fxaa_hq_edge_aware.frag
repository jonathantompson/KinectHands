...
)
{
  ...
  float lumaW = texture(tex_luma, f_texture + (vec2(-1.0, +0.0) * f_texel_size)).x;
	...
	float posVM, posVNW, posVNE, posVSW, posVSE, posVS, posVN, posVE, posVW;
    vec3 norM, norNW, norNE, norSW, norSE, norS, norN, norE, norW;
    SampleGBufferPoszNor(f_texture + (vec2(-1.0, -1.0) * f_texel_size), 
      f_depth_normal_view_stencil, posVNW, norNW);
    SampleGBufferPoszNor(f_texture + (vec2(+1.0, -1.0) * f_texel_size), 
	  f_depth_normal_view_stencil, posVNE, norNE);
    SampleGBufferPoszNor(f_texture + (vec2(-1.0, +1.0) * f_texel_size), 
	  f_depth_normal_view_stencil, posVSW, norSW);
    SampleGBufferPoszNor(f_texture + (vec2(+1.0, +1.0) * f_texel_size), 
	  f_depth_normal_view_stencil, posVSE, norSE);
    SampleGBufferPoszNor(f_texture + (vec2(+0.0, -1.0) * f_texel_size), 
      f_depth_normal_view_stencil, posVN, norN);
    SampleGBufferPoszNor(f_texture + (vec2(-1.0, +0.0) * f_texel_size), 
	  f_depth_normal_view_stencil, posVW, norW);
    SampleGBufferPoszNor(f_texture + (vec2(+0.0, +1.0) * f_texel_size), 
	  f_depth_normal_view_stencil, posVS, norS);
    SampleGBufferPoszNor(f_texture + (vec2(+1.0, +0.0) * f_texel_size), 
	  f_depth_normal_view_stencil, posVE, norE);
    SampleGBufferPoszNor(f_texture, f_depth_normal_view_stencil, posVM, norM);
    float max_delta_z = max(abs(posVM - posVSE), 
                            max(abs(posVM - posVSW), 
  						        max(abs(posVM - posVNE),
                                    max(abs(posVM - posVNW),
								        max(abs(posVM - posVN), 
									        max(abs(posVM - posVS),
										        max(abs(posVM - posVW),
											        abs(posVM - posVE)
	...
	bool earlyExit = range < rangeMaxClamped || rad_mult < 0.001;
	if(earlyExit)
		return rgbyM;

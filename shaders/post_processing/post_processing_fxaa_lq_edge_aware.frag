...
void main(){
  ...
  float lumaNW = texture2D(f_src_luma, f_texture.xy + (vec2(-0.5, -0.5) * f_texel_size)).x;
  float lumaNE = texture2D(f_src_luma, f_texture.xy + (vec2(+0.5, -0.5) * f_texel_size)).x;
  float lumaSW = texture2D(f_src_luma, f_texture.xy + (vec2(-0.5, +0.5) * f_texel_size)).x;
  float lumaSE = texture2D(f_src_luma, f_texture.xy + (vec2(+0.5, +0.5) * f_texel_size)).x;
  float lumaM  = texture2D(f_src_luma, f_texture.xy).x;
  ...
  vec3 rgbA = (1.0/2.0) * (
              texture2D(f_src_rgb, f_texture.xy + dir * (1.0/3.0 - 0.5)).xyz +
              texture2D(f_src_rgb, f_texture.xy + dir * (2.0/3.0 - 0.5)).xyz);
  vec3 rgbB = rgbA * (1.0/2.0) + (1.0/4.0) * (
              texture2D(f_src_rgb, f_texture.xy + dir * (0.0/3.0 - 0.5)).xyz +
              texture2D(f_src_rgb, f_texture.xy + dir * (3.0/3.0 - 0.5)).xyz);

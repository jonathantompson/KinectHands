...
#ifndef TESTS_TEST_UNIT_HEADER
#define TESTS_TEST_UNIT_HEADER
...
// Tests using printf --> Only works if values map to integers.
/*
#define EXPECT_TRUE(a) \
  if (!(a)) { \
  if (errors() == 0) printf("FAILED"); \
  printf("\n    Line %d Expected true %s: %d", __LINE__, # a, (a)); \
  incErrors(); \
  }
...
#define EXPECT_FALSE(a) \
  if (a) { \
  if (errors() == 0) printf("FAILED"); \
  printf("\n    Line %d Expected false %s: %d", __LINE__, # a, (a)); \
  incErrors(); \
  }
...
#define EXPECT_EQ(a, b) \
  if ((a) != (b)) { \
  if (errors() == 0) printf("FAILED"); \
  printf("\n    Line %d Expected equal %s and %s: (%f %f)", __LINE__, # a, \
    # b, (a), (b)); \
  incErrors(); \
  }
...
#define EXPECT_APPROX_EQ(a, b) \
  if (abs((a)-(b)) > 0.00001) { \
  if (errors() == 0) printf("FAILED"); \
  printf("\n    Line %d Expected approximately equal %s and %s: (%f %f)", \
    __LINE__, # a, # b, (a), (b)); \
  incErrors(); \
  }
...
#define EXPECT_NEQ(a, b) \
  if ((a) == (b)) { \
  if (errors() == 0) printf("FAILED"); \
  printf("\n    Line %d Expected not equal %s and %s: (%f %f)", __LINE__, \
    # a, # b, (a), (b)); \
  incErrors(); \
  }
...
#define EXPECT_GT(a, b) \
  if ((a) <= (b)) { \
  if (errors() == 0) printf("FAILED"); \
  printf("\n    Line %d Expected greater %s and %s: (%f %f)", __LINE__, \
    # a, # b, (a), (b)); \
  incErrors(); \
  }

#define EXPECT_FATAL(a) \
  TESTS_has_fatal_message = false; \
  (a); \
  if (!TESTS_has_fatal_message) { \
  if (errors() == 0) printf("FAILED"); \
  printf("\n    Line %d expected fatal %s", __LINE__, # a); \
  incErrors(); \
  }
*/

...
//  NOTE: ONLY FUNCTIONS THAT ARE USED HAVE BEEN PORTED
//
...
// int Effects::mask2 = createRGBMask(2);
// int Effects::mask4 = createRGBMask(4);
// int Effects::mask8 = createRGBMask(8);

/*

 **************************************************************
    ORIGIONAL & UNPORTED JAVA CODE --> PORT AS YOU NEED THEM
 **************************************************************
...
public static void enlarge(int src[], int dst[], int ws, int hs, int wd, int hd) {
  for (int y = 0 ; y < hs ; y++) {
    ...
    for (int x = 0 ; x < ws ; x++) {
      int i1x = x < ws-1 ? 1 : 0;
      dst[i2] = src[i1];
      dst[i2 + wd + 1] = avg(dst[i2 +  1] = avg(src[i1], src[i1 + i1x]),
...
public static int avg(int p1, int p2, int p3, int p4, int p5, int p6, int p7, int p8, int p9) {
  //return 0xff000000 | avg(p1, avg(avg(p2,p3,p4,p5), avg(p6,p7,p8,p9)));

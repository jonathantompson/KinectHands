...
namespace math {
  ...
  public:
    ...
    inline void trans_mult(Vec2* a, Mat2x2<T>* b) { trans_mult(this, a, b); }
    inline void trans_mult(Mat2x2<T>* a, Vec2* b) { trans_mult(this, a, b); }
    ...
    static void trans_mult(Vec2* ret, Vec2* a, Mat2x2<T>* b);  // this = a^t*b
    static void trans_mult(Vec2* ret, Mat2x2<T>* a, Vec2* b);  // this = a^t*b
  ...
  template <class T>
  void Vec2<T>::trans_mult(Vec2* ret, Vec2* a, Mat2x2<T>* b) {
    ret->m[0] = a->m[0]*b->m[0] + a->m[1]*b->m[2];
    ret->m[1] = a->m[0]*b->m[1] + a->m[1]*b->m[3];
  };
  template <class T>
  void Vec2<T>::trans_mult(Vec2* ret, Mat2x2<T>* a, Vec2* b) {
  ...
  bool Vec2<T>::equal(Vec2* a, Vec2 *b) {
    for (int i = 0; i < 2; i ++)
      if (std::abs((T)a->m[i] - (T)b->m[i]) > EPSILON)
        return false;
    return true;
  };

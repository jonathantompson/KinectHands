...
namespace kinect_interface_primesense {
  ...
  std::string KinectInterfacePrimesense::formatToString(const int mode) {
    ...
    default: 
      shutdownOpenNIStatic();
      throw std::wruntime_error("KinectInterfacePrimesense::formatToString() - ERROR: "
  ...
  openni::VideoMode KinectInterfacePrimesense::findMaxResYFPSMode(
    const openni::SensorInfo& sensor, const int required_format) const {
      ...
      if (ibest == -1) {
        shutdownOpenNIStatic();
        throw std::wruntime_error("KinectInterfacePrimesense::findMaxResYFPSMode() - "
  ...
  openni::VideoMode KinectInterfacePrimesense::findMatchingMode(
    ...
    const int fps, const int format) const {
      ...
      if (ibest == -1) {
        shutdownOpenNIStatic();
        throw std::wruntime_error("KinectInterfacePrimesense::findMatchingMode() - "
  ...
  void KinectInterfacePrimesense::initDepth() {
    ...
    if (depth_mode.getPixelFormat() != PixelFormat::PIXEL_FORMAT_DEPTH_1_MM) {
      throw std::wruntime_error("KinectInterfacePrimesense::init() - ERROR: "
  ...
  const uint8_t* KinectInterfacePrimesense::filteredDecisionForestLabels() const {
    return hand_detector_->labels_filtered();
  }

  const uint8_t* KinectInterfacePrimesense::rawDecisionForestLabels() const {

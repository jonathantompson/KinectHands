...
int32_t populateOccupancyList(int32_t** cur_occ, int32_t** next_occ, 
                              ImageData* data, int32_t max_pixels_per_image,
                              unsigned int* seed) {
  ...
  for (int32_t i = 0; i < data->num_images; i++) {
    ...
    for (uint32_t lab = 0; lab < NUM_LABELS; lab++) {
      int32_t count = label_count_cur_im[lab] > max_pixels_per_image ?
        max_pixels_per_image : label_count_cur_im[lab];
      num_points += count;
    }
  }

  *cur_occ = new int32_t[num_points];
  *next_occ = new int32_t[num_points];
  ...
  for (int32_t i = 0; i < data->num_images; i++) {
    ...
    // Perform a Knuth O(n) shuffle, but stop after the first max_pixels_per_image
    for (uint32_t lab = 0; lab < NUM_LABELS; lab++) {
      int32_t count = label_count_cur_im[lab] > max_pixels_per_image ?
        max_pixels_per_image : label_count_cur_im[lab];
      ...
      for (int32_t pix = 0; pix < count; pix++) {
        (*cur_occ)[index] = temp_space[(lab * im_size) + pix];
        index++;
...
int32_t populateOccupancyListBootstrap(int32_t** cur_occ, int32_t** next_occ, 
                                       ImageData* data, int32_t max_pixels_per_image,
                                       unsigned int* seed, DecisionTree* dt,
                                       uint32_t num_trees, uint32_t height) {
  // First count the number of starting pixels, these are pixels that the dt
  // evaluated incorrectly
  int32_t im_size = data->im_width * data->im_height;
  int32_t label_count_cur_im[NUM_LABELS];
  uint8_t* evaluated_label_data = new uint8_t[data->im_width * data->im_height];
  int32_t num_points = 0;
  for (int32_t i = 0; i < data->num_images; i++) {
    memset(label_count_cur_im, 0, NUM_LABELS * sizeof(label_count_cur_im[0]));
    evaluateDecisionForest(evaluated_label_data, dt, height, num_trees, 
      &data->image_data[i * im_size], data->im_width, data->im_height);

    // Get the label distribution for this image
    for (int32_t pix = 0; pix < im_size; pix++) {
      int32_t cur_index = (i * im_size) + pix;
      if (data->image_data[cur_index] != 0 && data->image_data[cur_index] < GDT_MAX_DIST) {
        if (data->label_data[cur_index] != evaluated_label_data[pix]) {
          uint8_t cur_label = data->label_data[cur_index];
          label_count_cur_im[cur_label]++;
        }
      }
...
//****************** MAIN ENTRY POINT **********************
//**********************************************************
void GenerateDecisionTree::generateDecisionTree(
  ...
  uint64_t data_length = settings->num_images_to_consider * 
                         training_data->im_width * training_data->im_height;
  if (data_length > 0x7fffffff) {  // MAX 32bit int
    throw std::runtime_error("ERROR: Data length is > int32_t address space!");
  }
  ...
  cout << "   width = " << training_data->im_width << endl;
  cout << "   height = " << training_data->im_height << endl;
  ...
  cout << endl << "DT#" << settings->dt_index << ": initializing occupancy list..." << endl;
  int32_t* cur_occupancy_list;
  int32_t* next_occupancy_list;
  if (bootstrap_dt != NULL && bootstrap_num_trees > 0) {
    // Create the occupancy structury by evaluating the previous decision tree
    // and putting mislabeled pixels onto the occupancy list.
    root.occupancy_length = populateOccupancyListBootstrap(&cur_occupancy_list, 
      &next_occupancy_list, training_data, settings->max_pixels_per_image_per_label, 
      &settings->seed, bootstrap_dt, bootstrap_num_trees, settings->tree_height);
  } else {
    ...
    root.occupancy_length = populateOccupancyList(&cur_occupancy_list, 
      &next_occupancy_list, training_data, settings->max_pixels_per_image_per_label, 
      &settings->seed);
  }
  ...
  if (wl_set->num_samples_per_node > num_wl_permutations) {
    wl_set->num_samples_per_node = num_wl_permutations;
  }
  ...
  for (int32_t i = 0; i < root.occupancy_length; i++) {
    hist_root[training_data->label_data[cur_occupancy_list[i]]]++;
  }
  ...
  while (!queue->empty()) {
    ...
    queue_cur_node = queue->read();
    bool changed_height = static_cast<int16_t>(cur_height) != queue_cur_node.height;
    if (changed_height) {
      // The BFS moved down a level, ping-pong the occupancy list buffers
      int32_t* tmp = cur_occupancy_list;
      cur_occupancy_list = next_occupancy_list;
      next_occupancy_list = tmp;
      cur_height = queue_cur_node.height;
    }
    int32_t* occ_sub_list = &cur_occupancy_list[queue_cur_node.occupancy_start];
...
#ifdef VERBOSE_GENERATION
      cout << "   trying weak learner (of " << wl_set->num_samples_per_node;
      cout << " attempts): 1 ";
#endif
#ifndef VERBOSE_GENERATION
    }
    ...
    max_gain = 0.0f;
    num_attempts = 0;
    while ((num_attempts < wl_set->num_samples_per_node) && 
      ...
      for (int32_t i = 0; i < queue_cur_node.occupancy_length; i++) {
        ...
        WL_FUNC(index, cur_wlu_offset, cur_wlv_offset, cur_threshold, 
          cur_wl_func, training_data->im_width, training_data->im_height, 
          training_data->image_data, result);

        if (result) {
          // Go left
          hist_left[training_data->label_data[index]]++;
        } else {
          // Go right
          hist_right[training_data->label_data[index]]++;
        }
      ...
      if (num_nodes_left > 0 && num_nodes_right > 0) {
        ...
        float cur_gain = queue_cur_node.entropy - 
          (entropy_left * num_nodes_left/static_cast<float>(queue_cur_node.occupancy_length) +
          entropy_right * num_nodes_right/static_cast<float>(queue_cur_node.occupancy_length));
    ...
    if (max_gain > 0) {
      ...
      int32_t* occ_sub_list_left = &next_occupancy_list[queue_left_node.occupancy_start];
      int32_t* occ_sub_list_right = &next_occupancy_list[queue_right_node.occupancy_start];
      ...
      if (cur_height < (settings->tree_height - 1)) {
        ...
        for (uint32_t i = 0; i < NUM_LABELS && add_left; i++) {
          if (static_cast<int32_t>(hist_left_best[i]) == queue_left_node.occupancy_length) {
            add_left = false;
          }
        }
        bool add_right = queue_right_node.occupancy_length != 0;
        for (uint32_t i = 0; i < NUM_LABELS && add_right; i++) {
          if (static_cast<int32_t>(hist_right_best[i]) == queue_right_node.occupancy_length) {
            add_right = false;
          }
        }
  ...
  // Clean up on finish
  delete queue;
  delete[] cur_occupancy_list;
  delete[] next_occupancy_list;

...
#define HAND_PTS_GROW_RAD_ITERATIONS 4
#define N_PTS_FILL 8

struct ImageData;

typedef Graph<float, float, float> GraphFloat;
...
namespace depth_images_io {
  ...
  public:
    ...
    uint32_t GetFilesInDirectory(data_str::VectorManaged<char*>& files_names, 
      std::string directory, bool load_processed_images);

    static void releaseImages(ImageData*& data);
    ...
    static void convertImageDepthToXYZ(float*& xyz, ImageData*& images);
  ...
  private:
    ...
    void AddVertex(GraphFloat* graph, uint32_t cur_ind, int* label_count);
    float CalculateAdjacentEdgeWeight(int ind0, int ind1, int16_t* image_data);
    static void graphErrorFunc(const char* err);
    ...
    GraphFloat* im_graph;
    GraphFloat::node_id vert[src_width * src_height];
    int queue_head;
    int queue_tail;
    int32_t delta_hsv[3];

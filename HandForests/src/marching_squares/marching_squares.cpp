...
namespace marching_squares {
  template <typename T>
  MarchingSquares<T>::MarchingSquares(T* mask, T threshold, uint32_t width, 
    uint32_t height) {
    width_ = width;
    height_ = height;

    marchingSquares(mask);
  }

  template <typename T>
  void MarchingSquares<T>::simplifyContour(uint32_t target_contour_count) {
    createMinHeap();
    cullContours(target_contour_count);
  }

  template <typename T>
  void MarchingSquares<T>::marchingSquares(T* mask) {
    ...
    for (uint32_t i = 0; i < contours_starts_.size(); i++) {
      ...
      do {
        ...
        while (cont != cont_next &&
               Float2::equal(&cont->v1, &cont_next->v1)) {
  ...
  template <typename T>
  void MarchingSquares<T>::appendContour(uint32_t image_index, 
  ...
  template <typename T>
  void MarchingSquares<T>::finishLastContour(uint32_t contour_start) {
     if (contours_.size() > 1) {
  ...
  template <typename T>
  void MarchingSquares<T>::cullContours(uint32_t target_contour_count) {
  ...
  template <typename T>
  void MarchingSquares<T>::printContours() {
    uint32_t j = 0;
    for (uint32_t i = 0; i < contours_starts_.size(); i++) {

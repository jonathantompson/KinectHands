...
const bool save_uncompressed_depth = false;
...
void loadImageForRendering() {
  char* file = im_files[cur_image];
  string full_filename = IMAGE_DIRECTORY + string(file);
  ...
  if (save_uncompressed_depth) {
    image_io->saveUncompressedDepth((IMAGE_DIRECTORY + string("depth_only_") +
      string(file)), cur_depth_data);
  }
}
...
void keyboard(unsigned char key, int x, int y) {
  ...
  case '=':
  case '+':
    if (cur_image < static_cast<int32_t>(im_files.size())-1) {
      cur_image++;
      image_changed = true;
    }
    cout << "depth image " << cur_image+1 << " of " << im_files.size() << endl;
  ...
  case 'S':
  case 's':
    cout << "SAVE IS NOT YET IMPLEMENTED!";
    break;
  case 'U':
  case 'u':
    image_changed = true;
...
int main(int argc, char *argv[]) { 
  ...
  cout << "+ - Go to the next image" << endl;
  cout << "- - Go to the prev image" << endl;
  cout << "p - Play images at <=" << 1.0 / PLAY_SPEED << "fps" << endl;
  ...
  try {
    ...
    if (image_io->GetFilesInDirectory(im_files, IMAGE_DIRECTORY, false) == 0) {

...
//
// 
// NOTE: THIS CIRCULAR BUFFER HAS BE MODIFIED TO NEVER OVERFLOW!

#ifndef data_str_CIRCULAR_BUFFER_HEADER
#define data_str_CIRCULAR_BUFFER_HEADER
...
namespace data_str {
  ...
  public:
    explicit CircularBuffer(uint32_t slots);
    ~CircularBuffer();
    ...
    // written, and marks slot as read.
    T& read();
  ...
  template <class T>
  CircularBuffer<T>::CircularBuffer(uint32_t slots) {
    if (slots < 1) {
  ...
  void CircularBuffer<T>::write(T& value) {
    ...
    if (p_write_ == p_read_) {
      ...
      throw std::runtime_error("CircularBuffer::write - buffer is full!");
    }
  };
  ...
  T& CircularBuffer<T>::read() {
    if (p_write_ == p_read_) {
      throw std::runtime_error("CircularBuffer::read - buffer is empty!");
    }
    uint32_t ret_index = p_read_;
    p_read_ = (p_read_ + 1) % size_;
    return arr_[ret_index];
  };
...
#endif  // data_str_CIRCULAR_BUFFER_HEADER

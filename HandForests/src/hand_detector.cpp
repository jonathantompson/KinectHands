...
using kinect::OpenNIFuncs;
using kinect::Vector3D;
...
HandDetector::HandDetector(const uint32_t im_width, const uint32_t im_height,
  ...
  uint32_t num_trees;
  loadForest(&forest_, &num_trees, filename);
  num_trees_ = (int32_t)num_trees;
  ...
  thread_cbs_ = new threading::Callback<void>*[HD_NUM_WORKER_THREADS+1];
...
void HandDetector::findHandLabelsFloodFill(const float* pt_hand_uvd, 
  ...
  float pt_hand_xyz[3];
  OpenNIFuncs::xnConvertProjectiveToRealWorld(1, (const Vector3D*)pt_hand_uvd, 
    (Vector3D*)pt_hand_xyz);
  ...
  OpenNIFuncs::xnConvertRealWorldToProjective(1, (const Vector3D*)pt_off_xyz, 
    (Vector3D*)pt_off_uvd);
